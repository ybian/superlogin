...
module.exports = function(config, router, passport, user) {
  ...
  router.post('/password-reset2', function (req, res, next) {
      user.resetPassword2(req.body, req)
        .then(function (user) {
          if(config.getItem('security.loginOnPasswordReset')) {
            return user.createSession(user._id, 'local', req.ip)
              .then(function (mySession) {
  ...
  router.post('/change-phone',
    passport.authenticate('bearer', {session: false}),
    function (req, res, next) {
      user.changePhone(req.user._id, req.body.newPhone, req)
        .then(function () {
          res.status(200).json({ok: true, success: 'Phone changed'});
        }, function (err) {
          return next(err);
        });
    });
