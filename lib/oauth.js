...
module.exports = function(router, passport, user, config) {
  ...
  function initSession(req, res, next) {
    ...
    return user.createSession(req.user._id, provider, req)
      ...
      .then(function (results) {
        ...
        } else if (req.query.cordova) {
          template = fs.readFileSync(path.join(__dirname, '../templates/oauth/auth-callback-cordova.ejs'), 'utf8');
        } else if (req.query.redirect) {
          results.redirect = req.query.redirect;
          results.redirectOnError = req.query.redirectOnError;
          template = fs.readFileSync(path.join(__dirname, '../templates/oauth/auth-callback-redirect.ejs'), 'utf8');
        } else {
  ...
  function registerTokenProvider (providerName, Strategy) {
    ...
    var configRef = 'providers.' + providerName + '-token';
    var providerConfig = config.getItem(configRef);
    if (!providerConfig) {
      providerConfig = config.getItem('providers.' + providerName);
    }
    if (providerConfig.credentials) {
      var credentials = providerConfig.credentials;
      credentials.passReqToCallback = true;
      var options = providerConfig.options || {};
