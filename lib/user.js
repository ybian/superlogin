...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.validatePhone = function (phone) {
    ...
    if (!phone.match(PHONE_REGEXP)) {
      return BPromise.resolve('invalid phone number');
    }
  ...
  function processInviteCode(userDoc) {
    ...
    var code = userDoc.inviteCode;
    return session._adapter.getKey(inviteCodePrefix + ':' + code)
      .then(function(result) {
        if(!result) {
          return BPromise.reject('invite code required');
        }
